...
//  /usr/local/Slicer-4.6/SlicerExecutionModel-build/GenerateCLP/bin/GenerateCLP --InputXML /home/leonardo/Desktop/Modulos-Source/Modulos_to_Slicer/TextureProcessing_V.8/source/TextureProcessing.xml --OutputCxx /home/leonardo/Desktop/Modulos-Source/Modulos_to_Slicer/TextureProcessing_V.8/Build/TextureProcessingCLP.h
//
#include <cstdio>
#include <cstdlib>
#include <iostream>
...
"	\"Contributor\" : \"Leonardo Machado (USP, RP), Luiz Otavio Murta Junior (USP, RP)\",\n"
...
"<executable>\n"
"  <category>FEATURE EXTRACTORS</category>\n"
...
"  <contributor>Leonardo Machado (USP, RP), Luiz Otavio Murta Junior (USP, RP)</contributor>\n"
"  <acknowledgements>This work is part of the required work for Master's Dessartation Defense.</acknowledgements>\n"
"  <parameters>\n"
"   <label>IO</label>\n"
...
#define GENERATE_TCLAP_PARSE \
    ...
    if (!std::string("Leonardo Machado (USP, RP), Luiz Otavio Murta Junior (USP, RP)").empty()) \
      { \
      fullDescription += "\nAuthor(s): Leonardo Machado (USP, RP), Luiz Otavio Murta Junior (USP, RP)"; \
      } \
    if (!std::string("This work is part of the required work for Master's Dessartation Defense.").empty()) \
      { \
      fullDescription += "\nAcknowledgements: This work is part of the required work for Master's Dessartation Defense."; \
      } \
 ...
 \
    msg.str("");msg << "Allows it to do a normalization of pixel intensities in the image before feature extraction. (default: " << doNormalization << ")"; \
    TCLAP::SwitchArg doNormalizationArg("n", "donormalization", msg.str(), commandLine, doNormalization); \
 \
    msg.str("");msg << "Allows it to extract histogram features. (default: " << doHistogramFeat << ")"; \
    TCLAP::SwitchArg doHistogramFeatArg("d", "dohistogramfeat", msg.str(), commandLine, doHistogramFeat); \
 \
    msg.str("");msg << "Allows it to calculate and save percentile values. (default: " << doPercentiles << ")"; \
    TCLAP::SwitchArg doPercentilesArg("p", "dopercentiles", msg.str(), commandLine, doPercentiles); \
 \
    msg.str("");msg << "Allows it to extract coocurrence matrix features. (default: " << doCoocurrenceFeat << ")"; \
    TCLAP::SwitchArg doCoocurrenceFeatArg("c", "docoocurrencefeat", msg.str(), commandLine, doCoocurrenceFeat); \
 \
    msg.str("");msg << "Allows it to extract run length matrix features. (default: " << doRunFeat << ")"; \
    TCLAP::SwitchArg doRunFeatArg("r", "dorunfeat", msg.str(), commandLine, doRunFeat); \
 \
    msg.str("");msg << "Allows it to extract gradient features. (default: " << doGradient << ")"; \
    TCLAP::SwitchArg doGradientArg("g", "dogradient", msg.str(), commandLine, doGradient); \
 \
    msg.str("");msg << "Allows it to extract shape features. (default: " << doShapeFeat << ")"; \
    TCLAP::SwitchArg doShapeFeatArg("s", "doshapefeat", msg.str(), commandLine, doShapeFeat); \
 \
    msg.str("");msg << "This will plot the histogram in a .txt file. (default: " << doHistoPlot << ")"; \
    ...
    while (ac < static_cast<size_t>(argc)) \
      ...
      /* short flag case && long flag case */ \
       if ((strlen(argv[ac]) == 2 && argv[ac][0]=='-') \
         ...
         bool isPresent = dvOptionnalArgsIt != deserializedVectorFlaggedArgs.end(); \
         if (isBoolean) \
           { \
           ...
           bool isPresent = dvMultipleArgsIt != deserializedMultipleArgsMap.end(); \
           /*Ignore if boolean and already present*/ \
           /*Reset/Add the value if first deserialize or not present*/ \
           if (!isPresent || !multipleFlags[flag]) \
             { \
